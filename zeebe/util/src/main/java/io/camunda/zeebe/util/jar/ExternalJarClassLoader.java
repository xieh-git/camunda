...
public final class ExternalJarClassLoader extends URLClassLoader {
  ...
  public void close(final boolean shouldWarn) throws IOException {
    if (shouldWarn) {
      LOGGER.warn(
          """
          Closing the external JAR class loader may lead to ClassNotFoundException;
          call #close(false) if you want to close it explicitly""");
    }
