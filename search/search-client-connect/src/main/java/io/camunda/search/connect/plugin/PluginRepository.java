...
public final class PluginRepository implements AutoCloseable {
  ...
  private final SequencedMap<String, Class<? extends DatabaseCustomHeaderSupplier>> plugins;
  ...
  PluginRepository(
      final SequencedMap<String, Class<? extends DatabaseCustomHeaderSupplier>> plugins,
  ...
  public SequencedMap<String, Class<? extends DatabaseCustomHeaderSupplier>> getPlugins() {
  ...
  Stream<DatabaseCustomHeaderSupplier> instantiate() {
    return plugins.entrySet().stream().map(entry -> instantiate(entry.getKey(), entry.getValue()));
  }
  ...
  private DatabaseCustomHeaderSupplier instantiate(
  ...
  private DatabaseCustomHeaderSupplier instrumented(final DatabaseCustomHeaderSupplier supplier) {
    return () ->
        ThreadContextUtil.supplyWithClassLoader(
            supplier::getCustomHttpHeader, supplier.getClass().getClassLoader());
  }
}
