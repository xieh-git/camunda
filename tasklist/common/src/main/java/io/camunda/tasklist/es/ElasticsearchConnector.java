...
public class ElasticsearchConnector {
  ...
  private final PluginRepository esPluginRepository;
  private final PluginRepository zeebeEsPluginRepository;

  private ElasticsearchClient elasticsearchClient;

  @Autowired
  public ElasticsearchConnector(final TasklistProperties tasklistProperties) {
    this.tasklistProperties = tasklistProperties;
    esPluginRepository =
        new PluginRepository().load(tasklistProperties.getElasticsearch().getPlugins());
    zeebeEsPluginRepository =
        new PluginRepository().load(tasklistProperties.getZeebeElasticsearch().getPlugins());
  }
    ...
    final RestClient restClient =
        restClientBuilder
            .setHttpClientConfigCallback(
                httpClientBuilder ->
                    configureHttpClient(httpClientBuilder, elsConfig, esPluginRepository))
            .build();
  ...
  public RestHighLevelClient tasklistEsClient() {
    ...
    return createEsClient(tasklistProperties.getElasticsearch(), esPluginRepository);
  }
  ...
  public RestHighLevelClient tasklistZeebeEsClient() {
    ...
    // https://discuss.elastic.co/t/elasticsearch-5-4-1-availableprocessors-is-already-set/88036/3
    System.setProperty("es.set.netty.runtime.available.processors", "false");
    return createEsClient(tasklistProperties.getZeebeElasticsearch(), zeebeEsPluginRepository);
  }
  ...
  public RestHighLevelClient createEsClient(
    ...
    final RestClientBuilder restClientBuilder =
        RestClient.builder(getHttpHost(elsConfig))
            .setHttpClientConfigCallback(
                httpClientBuilder ->
                    configureHttpClient(httpClientBuilder, elsConfig, pluginRepository));
    if (elsConfig.getConnectTimeout() != null || elsConfig.getSocketTimeout() != null) {
      restClientBuilder.setRequestConfigCallback(
          configCallback -> setTimeouts(configCallback, elsConfig));
    }
  ...
  private HttpAsyncClientBuilder configureHttpClient(
    ...
    if (!pluginRepository.isEmpty()) {
      httpAsyncClientBuilder.addInterceptorLast(pluginRepository.asRequestInterceptor());
    }
